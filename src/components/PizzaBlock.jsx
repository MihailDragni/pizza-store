import React, { useState } from 'react';
import { useDispatch } from 'react-redux';
import { addItem } from '../redux/slices/cart-slice';
import { useSelector } from 'react-redux';
import { Link } from 'react-router-dom';
const tyNames = ['тонкое', 'традиционное'];

const PizzaBlock = ({ item }) => {
  const dispatch = useDispatch();
  const cartItem = useSelector((state) => state.cart.items.find((obj) => obj.id === item.id));
  const [typeActive, setTypeActive] = useState(0);
  const [listActive, setListActive] = useState(0);

  const addedItem = cartItem ? cartItem.count : 0;

  const addToCartItem = () => {
    const thing = {
      id: item.id,
      title: item.title,
      price: item.price,
      imageUrl: item.imageUrl,
      type: tyNames[typeActive],
      size: item.sizes[listActive],
    };

    dispatch(addItem(thing));
  };

  return (
    <div className="pizza-block-wrapper">
      <div className="pizza-block">
        <Link to={`/pizzas/${item.id}`}>
          <img className="pizza-block__image" src={item.imageUrl} alt="Pizza" />
        </Link>
        <h4 className="pizza-block__title">{item.title}</h4>
        <div className="pizza-block__selector">
          <ul>
            {item.types.map((typeId) => (
              <li
                key={typeId}
                onClick={() => setTypeActive(typeId)}
                className={typeActive === typeId ? 'active' : ''}>
                {tyNames[typeId]}
              </li>
            ))}
          </ul>
          <ul>
            {item.sizes.map((item, index) => (
              <li
                key={index}
                onClick={() => setListActive(index)}
                className={listActive === index ? 'active' : ''}>
                {item} см.
              </li>
            ))}
          </ul>
        </div>
        <div className="pizza-block__bottom">
          <div className="pizza-block__price">от {item.price} ₽</div>
          <button onClick={addToCartItem} className="button button--outline button--add">
            <svg
              width="12"
              height="12"
              viewBox="0 0 12 12"
              fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path
                d="M10.8 4.8H7.2V1.2C7.2 0.5373 6.6627 0 6 0C5.3373 0 4.8 0.5373 4.8 1.2V4.8H1.2C0.5373 4.8 0 5.3373 0 6C0 6.6627 0.5373 7.2 1.2 7.2H4.8V10.8C4.8 11.4627 5.3373 12 6 12C6.6627 12 7.2 11.4627 7.2 10.8V7.2H10.8C11.4627 7.2 12 6.6627 12 6C12 5.3373 11.4627 4.8 10.8 4.8Z"
                fill="white"
              />
            </svg>
            <span>Добавить</span>
            {addedItem > 0 && <i>{addedItem}</i>}
          </button>
        </div>
      </div>
    </div>
  );
};

export default PizzaBlock;
